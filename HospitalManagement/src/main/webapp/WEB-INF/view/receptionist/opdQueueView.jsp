	...
	<div class="row">
	    ...
	    <div class="col-sm-12">
		  ...
		  <table class="table table-striped table-bordered">
		    ...
		    <tbody>
			    <%
			    	List<_OpdRecord> opds=(List<_OpdRecord>) request.getAttribute("opdQueue");
			    	for(_OpdRecord rec: opds)
			    	{
			    %>
		       <tr>
		      	<td><%= rec.getPid() %></td>
		      	<td><%= rec.getPatientName() %>
		        <td><%= rec.getDoctorId() %></td>
		        <td><%= rec.getDoctorName() %></td>	
		        
		      	<td> <span  style="text-align:center;">
		      		<form action="searchPatientById.html" method="post">
				   		<input type="text" name="id" value="<%= rec.getPid() %>" hidden="">
				    	<button type="submit" class="btn btn-primary"> View </button>
				    </form></span>   
		      	</td>
		      	
		      	<td><span  style="text-align:center;">
		      		<form action="deleteOpd.html" method="post">
		      			<input type="text" name="pid" value="<%= rec.getPid()%>" hidden="">
